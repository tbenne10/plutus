...
genStateTip TxGenState {_txgsBlocks, _txgsNumTransactions} =
    Tip
        ...
        , tipBlockId = BlockId $ BSL.toStrict $ serialise numBlocks -- TODO Incl. hash of utxo set!
...
deleteInputs :: forall effs. Member (State TxGenState) effs => Set TxOutRef -> Eff effs ()
deleteInputs spent = do
  modify (over txgsUtxoSet (\s -> s `Set.difference` spent))
...
genTxState = sendM genChainAction >>= \case
    ...
    AddTx     -> do
      ...
      pure ( TxUtxoBalance.fromTx tx
           ...
           , TxIdState.fromTx (BlockNumber blockNumber) tx
           )
