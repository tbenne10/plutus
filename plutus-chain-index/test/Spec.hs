...
txOutStatusTxStatusProp = property $ do
  ...
  let blockNumber = BlockNumber numBlocks

  -- Combine spent and unspent outptus
  let txOutputs = view Gen.txgsUtxoSet txGenState <> view Gen.txgsStxoSet txGenState

  forM_ txOutputs $ \txOutRef@TxOutRef{ txOutRefId } -> do
    let txStatusE = transactionStatus blockNumber txIdState txOutRefId
    let txOutStatusE = TxOutBalance.transactionOutputStatus blockNumber
                                                            txIdState
                                                            txOutBalance
                                                            txOutRef
    case (txStatusE, txOutStatusE) of
      (Right txStatus, Right txOutStatus) -> do
        txStatus === liftTxOutStatus txOutStatus
      (Left _, Right _) -> Hedgehog.assert False
      _ -> Hedgehog.assert True
...
txOutStatusSpentUnspentProp = property $ do
  ...
  forM_ (view Gen.txgsStxoSet txGenState) $ \txOutRef@TxOutRef { txOutRefId } -> do
    let txOutStatus = TxOutBalance.transactionOutputStatus blockNumber
                                                  txIdState
                                                  txOutBalance
                                                  txOutRef
    case txOutStatus of
      Right (TentativelyConfirmed _ TxValid (Spent txId)) | txId == txOutRefId -> Hedgehog.assert True
      _                                                                        -> Hedgehog.assert False
